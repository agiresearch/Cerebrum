...
def setup_client(
    ...
    client = Cerebrum(aios_kernel_url)
...
def run_agent(
    ...
    try:
        client.connect()
        print(f"🚀 Executing agent: {os.path.basename(agent_name)}")
        print(f"📋 Task: {task}")
        
        result = client.execute(agent_name, {"task": task, "refresh": refresh})
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        help="Name of the agent to execute, for example: demo_author/demo_agent"
    )
    parser.add_argument(
        "--task",
        required=True,
